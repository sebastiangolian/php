<?php
namespace sebastiangolian\php\writer;
...
class Xlsx
{
    ...
    protected function initializeSheet($sheet_name, $col_widths = array(), $auto_filter = false, $freeze_rows = false, $freeze_columns = false)
    {
        ...
        $this->sheets[$sheet_name] = (object) array(
            ...
            'row_count' => 0,
            'file_writer' => new XlsxBuffererWriter($sheet_filename),
            'columns' => array(),
            'merge_cells' => array(),
        ...
        $max_cell = Xlsx::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL); // XFE1048577
    ...
    protected function writeCell(XlsxBuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
    {
    ...
    protected function writeStylesXML()
    {
        ...
        $file = new XlsxBuffererWriter($temporary_filename);
...
class XlsxBuffererWriter
{
    ...
    public function __construct($filename, $fd_fopen_flags = 'w', $check_utf8 = false)
    {
        ...
        if ($this->fd === false) {
            self::log("Unable to open $filename for writing.");
        }
    }
    ...
    protected function purge()
    {
        if ($this->fd) {
            if ($this->check_utf8 && ! self::isValidUTF8($this->buffer)) {
                self::log("Error, invalid UTF8 encoding detected.");
